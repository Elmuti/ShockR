...
local Orakel = require(game.ReplicatedStorage.Abyss.Main)
        ...
        for i, child in pairs(brick:GetChildren()) do
                if child.ClassName == "ObjectValue" and child.Name == "connection" then
					if child.Value == nil then
						error("Connection didn't have a node reference (it was nil), This usually happens when a node is removed without removing its connections. Fix your nodegraph.")
					end
				...
				--elseif child.Name == "node_cover" then
...
function pathLib.GetNearestNodeVis(subject, position, visRange, returnBrick, dir, masterTable, ignore)
    local nodeDir = dir
    if type(dir) ~= "table" then
            nodeDir = dir:children()
    end
	...
	local ray = Ray.new(subject.Position,(nearest.Position - subject.Position).unit * visRange)
	local lineOfSight = workspace:FindPartOnRayWithIgnoreList(ray, ignore)
...
function pathLib.GetNodesVisibleInRadius(position, radius, returnBrick, dir, masterTable, ignore)
	local list = {}
	...
	for _, node in pairs(dir:GetChildren()) do
		...
		if dist <= radius then
			...
			local hit, pos = workspace:FindPartOnRayWithIgnoreList(ray, ignore or {})
			if hit ~= nil then
				if hit == node then
					if returnBrick then
...
function pathLib.AStar(masterTable, startID, endID, takeSafest)
	local takeSafestPath = takeSafest or false
    local now = tick()
    local closed = {}
    local open = {startID}
    local previous = {}
